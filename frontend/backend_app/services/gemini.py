...
def pydantic_to_gemini_schema(model_class):
    """
    Converts a Pydantic model class to a raw schema dictionary for Gemini API.
    Resolves/inlines all nested references ($ref) and strips out all 'default'
    and 'title' metadata keys to prevent the API from rejecting the request with:
    'Unknown field for Schema: default' or 'Unknown field for Schema: title'
    """
    if hasattr(model_class, "model_json_schema"):
        raw_schema = model_class.model_json_schema()
    else:
        raw_schema = model_class.schema()
        
    defs = raw_schema.get("$defs") or raw_schema.get("definitions") or {}
    
    def resolve_and_clean(schema_item):
        ...
        if "$ref" in schema_item:
            ...
            return resolve_and_clean(dict(resolved))
        ...
        for k, v in schema_item.items():
            ...
            if k == "title" and isinstance(v, str):
                continue
            if k == "anyOf" or k == "oneOf":
                types = [x for x in v if isinstance(x, dict) and x.get("type") != "null"]
                if types:
                    return resolve_and_clean(types[0])
                continue
            if k == "properties" and isinstance(v, dict):
                cleaned[k] = {prop_name: resolve_and_clean(prop_schema) for prop_name, prop_schema in v.items()}
            elif isinstance(v, dict):
                cleaned[k] = resolve_and_clean(v)
            elif isinstance(v, list):
                cleaned[k] = [resolve_and_clean(item) for item in v]
            else:
                cleaned[k] = v
        return cleaned

    final_schema = resolve_and_clean(raw_schema)
    if "$defs" in final_schema:
        del final_schema["$defs"]
    if "definitions" in final_schema:
        del final_schema["definitions"]
...
def evaluate_speech_session(
    ...
    try:
        response = call_generative_model(
            ...
            generation_config={
                "response_mime_type": "application/json",
                "response_schema": pydantic_to_gemini_schema(SpeechEvaluation),
            }
        )
...
def generate_follow_up_question(
    ...
    try:
        response = call_generative_model(
            ...
            generation_config={
                "response_mime_type": "application/json",
                "response_schema": pydantic_to_gemini_schema(FollowUpQuestionResponse),
            }
        )
        data = json.loads(response.text)
...
def evaluate_round_response(
    ...
    try:
        response = call_generative_model(
            ...
            generation_config={
                "response_mime_type": "application/json",
                "response_schema": pydantic_to_gemini_schema(RoundEvaluation),
            }
        )
        data = json.loads(response.text)
...
def generate_final_interview_evaluation(
    ...
    try:
        response = call_generative_model(
            ...
            generation_config={
                "response_mime_type": "application/json",
                "response_schema": pydantic_to_gemini_schema(FinalInterviewEvaluation),
            }
        )
        data = json.loads(response.text)
...
def generate_coach_report(
    ...
    try:
        response = call_generative_model(
            ...
            generation_config={
                "response_mime_type": "application/json",
                "response_schema": pydantic_to_gemini_schema(CoachReport),
            }
        )
        data = json.loads(response.text)
