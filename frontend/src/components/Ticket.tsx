...
export const Ticket: React.FC<TicketProps> = ({
  ...
  const getAnimationProps = (): TargetAndTransition => {
    ...
    return { x: 0, y: -300, z: 0, rotateX: 0, rotateZ: 0, skewX: 0, scaleX: 1, scaleY: 1, opacity: 1 };
  };

  return (
    <motion.div
      id="dispenser-ticket"
      className="relative w-[240px] sm:w-[260px] cursor-default select-none will-change-transform"
      style={{
      ...
      initial={
        status === 'ready'
          ...
          : { x: 0, y: -300, z: 0, rotateX: 2, rotateZ: 0, skewX: 0, scaleX: 1, scaleY: 0.99, opacity: 1 }
      }
    ...
    >
      <div className="w-full relative">
        ...
        <div className="w-full bg-white rounded-t-xs border-l border-r border-[#ECE6DE] overflow-hidden relative">
          ...
          {/* Ticket Body Content */}
          <div className="px-3 py-2 sm:px-3.5 sm:py-2.5 text-center flex flex-col items-center justify-between min-h-[210px] sm:min-h-[225px] relative bg-gradient-to-b from-[#FFFFFF] via-[#FCFCFA] to-[#FAF8F5]">
            {/* Header matching image: ✦ YOUR TOPIC IS READY! ✦ */}
            <div className="w-full flex flex-col items-center pt-0 pb-1">
              <div className="flex items-center justify-center gap-1.5 text-[#FF4A57]">
                <span className="text-xs font-bold select-none leading-none">✦</span>
                <span className="text-[11px] sm:text-xs font-extrabold tracking-wider uppercase font-sans">
                  YOUR TOPIC IS READY!
                </span>
                <span className="text-xs font-bold select-none leading-none">✦</span>
              </div>
              <div className="w-full h-[1px] bg-[#FF4A57]/20 mt-1.5 mb-0.5" />
            </div>

            {/* Category Pill */}
            <div className="text-[9px] sm:text-[10px] font-semibold tracking-widest uppercase text-[#A0988E] mb-0.5 font-mono">
              {topic.categoryLabel}
            </div>
            ...
            <div className="px-1 py-0.5 w-full">
              <h2
                ...
                className="text-sm sm:text-[15px] font-bold text-[#1F1E24] leading-snug tracking-tight font-sans"
              >
                "{topic.text}"
              </h2>
            </div>
            ...
            {topic.talkingPoints && topic.talkingPoints.length > 0 && (
              <div className="w-full text-left my-1 px-0.5">
                <div className="flex items-center gap-1.5 mb-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#FF4A57]" />
                  <span className="text-[9px] font-extrabold uppercase tracking-wider text-[#FF4A57] font-mono">
                ...
                <div className="flex flex-col gap-1 w-full">
                  {topic.talkingPoints.slice(0, 3).map((point, idx) => (
                    <div
                      key={idx}
                      className="flex items-start gap-1.5 bg-[#F9F7F3] px-2 py-1 rounded-md border border-[#EDE6DC]"
                    >
                      <span className="flex-shrink-0 w-3.5 h-3.5 rounded-full bg-[#FF4A57]/12 text-[#FF4A57] text-[9px] font-bold flex items-center justify-center font-mono mt-0.5">
                        {idx + 1}
                      </span>
                      <p className="text-[10px] sm:text-[11px] text-[#38332E] font-medium leading-tight font-sans line-clamp-2">
                        {point}
                      </p>
                    </div>
                  ))}
            ...
            <div className="w-full pt-1.5 mt-0.5 border-t border-dashed border-[#E8E2D8] flex items-center justify-between text-xs text-[#7A7267]">
              <div className="flex items-center gap-1.5 font-medium">
                <div className="p-0.5 rounded-full bg-[#F3EFEA] text-[#FF4A57]">
                  <Clock className="w-3 h-3" />
                </div>
                <span className="text-[10px] sm:text-[11px]">{topic.timeToSpeak || 90}s to speak</span>
              </div>
